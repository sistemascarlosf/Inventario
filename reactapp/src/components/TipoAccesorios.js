...
import { showAllTipoAccesorios } from "../servicios/tipoAccesorios";
...
function TipoAccesorios() {
    ...
    const getDatos = async () => {
        const response = await showAllTipoAccesorios();

        if (`${response.status}` === process.env.REACT_APP_RESPONSE_CODE_OK) {
            const resTipoAccesorios = response.data;
            setData(resTipoAccesorios);
        } else {
            console.log("error");
        }
    }
